...
class Engine{
  constructor(){
    ...
    this.stem=null;
    // the worklet-based stages arrive a moment after init(); until then the built-in nodes stand in
    this.wl=null;this.worklets=false;this.reverbSettings=Object.assign({},(Z.REVERB&&Z.REVERB.dflt)||{size:60,damp:40,pre:15});
    // sample-based drum voices, per kind, and each voice's own pitch (semitones) and decay (percent of full)
    this.samples={};this.voice={};this.choked=null;
  }
  voiceOf(kind){return this.voice[kind]||{pitch:0,decay:100}}
  init(ctxIn){
    if(this.ctx)return;
    ...
    this.lim2=ctx.createGain(); // the lookahead limiter slots in here once its worklet has loaded
    this.comp.connect(this.limiter);this.limiter.connect(this.lim2);this.lim2.connect(this.fade);this.fade.connect(ctx.destination);this.fade.connect(this.analyser);
    ...
    this.wl=(Z.installWorklets?Z.installWorklets(ctx):Promise.resolve(false)).then(ok=>{if(ok)this.attachWorklets();return ok}).catch(()=>false);
  }
  // swap the stand-ins for the real thing: the lookahead limiter after the bus compressor, the FDN reverb
  // in place of the convolver. Both keep the same connections, so nothing else in the chain notices.
  attachWorklets(){
    const ctx=this.ctx;if(!ctx||this.worklets)return;
    try{
      const lim=new AudioWorkletNode(ctx,'zinth-limiter',{outputChannelCount:[2]});
      lim.parameters.get('ceiling').value=Math.pow(10,((Z.LOUD&&Z.LOUD.ceiling)||-1)/20);
      this.limiter.disconnect(this.lim2);this.limiter.connect(lim);lim.connect(this.lim2);this.limNode=lim;
      const rv=new AudioWorkletNode(ctx,'zinth-reverb',{outputChannelCount:[2]});
      this.reverbIn.disconnect(this.reverb);this.reverbIn.connect(rv);rv.connect(this.reverbOut);this.reverbNode=rv;
      this.worklets=true;this.setReverb(this.reverbSettings);
    }catch(e){this.worklets=false}
  }
  // the room: size, damping and pre-delay, in 0..100 and milliseconds
  setReverb(r){
    this.reverbSettings=Object.assign(this.reverbSettings,r||{});
    const n=this.reverbNode;if(!n)return;const s=this.reverbSettings,t=this.ctx.currentTime;
    n.parameters.get('size').setTargetAtTime(Math.max(0,Math.min(1,s.size/100)),t,0.05);
    n.parameters.get('damp').setTargetAtTime(Math.max(0,Math.min(1,s.damp/100)),t,0.05);
    n.parameters.get('predelay').setTargetAtTime(Math.max(0,Math.min(0.12,s.pre/1000)),t,0.05);
  }
  ...
  playNote(L,midi,vel,time,durSec,pan,busName){
    ...
    const mkChain=()=>{
      // the ladder: a worklet per chain, with the same envelope on its cutoff; it ends itself when the voice does
      if(p.slope===36&&this.worklets){
        try{const n=new AudioWorkletNode(ctx,'zinth-ladder');n.parameters.get('reso').value=Math.max(0,Math.min(1,p.reso/100));filterEnv({frequency:n.parameters.get('cutoff')});return {in:n,out:n}}catch(e){}
      }
  ...
  playDrum(kind,vel,time){
    ...
    const vo=this.voiceOf(kind),pt=Math.pow(2,(vo.pitch||0)/12),dm=Math.max(0.05,(vo.decay===undefined?100:vo.decay)/100);
    if(kind==='kick'){const pump=(this.params.drums.pump||0)/100;
    ...
    // a closed hat chokes the open one, sampled or synthesised, the way a real hi-hat does
    if(kind==='hat'&&this.choked){try{this.choked.stop(time)}catch(e){}this.choked=null}
    // a sample takes the voice over completely: pitched, shortened, and choked like the synth voice would be
    const smp=this.samples[kind];
    if(smp&&smp.buffer){
      const s=ctx.createBufferSource();s.buffer=smp.buffer;s.playbackRate.value=pt;
      const full=smp.buffer.duration/pt,dec=Math.max(0.03,full*dm),g=ctx.createGain();
      g.gain.setValueAtTime(vel*0.9,time);if(dm<0.999){g.gain.setValueAtTime(vel*0.9,time+dec*0.7);g.gain.linearRampToValueAtTime(0.0001,time+dec)}
      s.connect(g);g.connect(bus);s.start(time);s.stop(time+dec+0.05);
      if(kind==='ohat')this.choked=s;
      return;
    }
    if(kind==='kick'){
      const k=K.kick;
      const o=ctx.createOscillator(),g=ctx.createGain();o.type='sine';
      o.frequency.setValueAtTime(k.f0*pt,time);o.frequency.exponentialRampToValueAtTime(k.f1*pt,time+0.09);
      g.gain.setValueAtTime(vel*0.95,time);g.gain.exponentialRampToValueAtTime(0.001,time+k.decay*dm);
      ...
      o.connect(g);g.connect(dest);o.start(time);o.stop(time+k.decay*dm+0.05);
      ...
      const g=ctx.createGain();g.gain.setValueAtTime(vel*k.noise*1.15,time);g.gain.exponentialRampToValueAtTime(0.001,time+k.decay*dm);
      n.connect(f);f.connect(hp);hp.connect(g);g.connect(bus);n.start(time);n.stop(time+k.decay*dm+0.03);
      // the body: two tuned oscillators, the fundamental and the shell's overtone, both dropping in pitch
      [[k.tone*pt,0.35,0.11*dm],[k.tone*pt*1.83,0.18,0.07*dm]].forEach(([hz,lv,dur])=>{
      ...
      const k=K.hat,open=kind==='ohat',dur=(open?k.open:k.decay)*dm;
      ...
      const f=ctx.createBiquadFilter();f.type='highpass';f.frequency.value=Math.min(16000,k.hp*pt);f.connect(g);
      // the open hat is choked by the next closed one: its gain is what gets cut
      const choke={stop:t=>{g.gain.cancelScheduledValues(t);g.gain.setValueAtTime(g.gain.value,t);g.gain.exponentialRampToValueAtTime(0.0001,t+0.02)}};
      if(open)this.choked=choke;
      if(k.metal){
        // the metallic recipe: six square waves at inharmonic ratios through a bandpass, plus a little noise
        const bp=ctx.createBiquadFilter();bp.type='bandpass';bp.frequency.value=Math.min(16000,k.hp*1.3*pt);bp.Q.value=1.1;
        const mg=ctx.createGain();mg.gain.value=0.85;bp.connect(mg);mg.connect(f);
        for(const hz of HAT_PARTIALS){const o=ctx.createOscillator();o.type='square';o.frequency.value=hz*(k.tone||1)*pt;o.connect(bp);o.start(time);o.stop(time+dur+0.02)}
  ...
  start(section,countIn){
    ...
    if(this.onTransport)this.onTransport(true);
  }
  ...
  stop(){clearInterval(this.timer);this.timer=null;this.playing=false;this.queue=[];this.glideFrom={};this.countInEnd=0;if(this.ctx){for(const n of ['lp','hp','crush','throw','wash','gate8','gate16'])this.fxOff(n);this.resetSweep();this.resetFade()}if(this.onTransport)this.onTransport(false)}
  ...
  tick(){
    ...
    while(this.grid<ctx.currentTime+0.16){
      ...
      if(this.onStep)this.onStep(this.step,t,d);
    ...
    for(const L of LAYERS){
      ...
      for(let i=0;i<evs.length;i++){
        ...
        if(L==='drums'){
          ...
          this.playDrum(e.kind,lay[L]==='lite'?vel*0.6:vel,t);if(this.onDrum)this.onDrum(e.kind,lay[L]==='lite'?vel*0.6:vel,t);
        }else if(L==='chords'){
          const n=e.notes.length,dur=e.dur*d-0.02;e.notes.forEach((m,k)=>{this.playNote(L,m,vel,t,dur,(k/(n-1||1)-0.5)*0.5);if(this.onNote)this.onNote(L,m,vel,t,dur)});
        }else if(L==='arp'){
          const dur=Math.max(0.05,e.dur*d*0.95);this.playNote(L,e.midi,vel,t,dur,e.midi%2?0.3:-0.3);if(this.onNote)this.onNote(L,e.midi,vel,t,dur);
        }else{
          const dur=Math.max(0.05,e.dur*d*0.95);
          this.playNote(L,e.midi,vel,t,dur,L==='lead'?0.08:0);if(this.onNote)this.onNote(L,e.midi,vel,t,dur);
