...
// song-length presets: Short, Radio and Extended rebuild the arrangement into a whole song of about that
// long at the tempo you are on. A preset may only ever build a song — one that opens, has a verse and a
// chorus and ends — out of the rungs of the ladder; it must take the rung that lands closest to the length
// it promises, come near that length at every tempo the slider can reach, and never build a shorter song
// than the preset below it. And because it swaps nothing but the sections, every note generated over the
// new form must still be in key, and the form must come back from a link exactly as it went in.
{
  const tag=' [song length]';
  const ids={};
  Z.SONG_LENGTHS.forEach((p,i)=>{
    assert(!!(p.id&&p.label&&p.says),'a song-length preset is missing its name'+tag);
    assert(!ids[p.id],'two song-length presets share the id '+p.id+tag);ids[p.id]=1;
    assert(p.seconds>=30&&p.seconds<=600,'song-length preset '+p.id+' asks for '+p.seconds+' s'+tag);
    if(i)assert(p.seconds>Z.SONG_LENGTHS[i-1].seconds,'the song-length presets are out of order'+tag);
    assert(Z.songLengthOf(p.id)===p,'song-length preset '+p.id+' cannot be looked up'+tag);
  });
  assert(!Z.songLengthOf('nonsense'),'a song length that does not exist was found anyway'+tag);
  // the clock a length is read off: always m:ss, and never the 1:60 that rounding alone would give
  assert(Z.clockOf(0)==='0:00'&&Z.clockOf(59.4)==='0:59'&&Z.clockOf(119.7)==='2:00'&&Z.clockOf(305)==='5:05','the song clock does not read m:ss'+tag);
  // every rung of the ladder is a whole song, longer than the rung under it, made of real sections
  let prevBars=0;
  for(let n=0;n<=Z.FORM_MAX_STAGE;n++){
    const form=Z.formAt(n),bars=Z.formBars(form),t=' [form rung '+n+']';
    assert(form.length>=4,'a form of '+form.length+' sections is not a song'+t);
    form.forEach(e=>{
      const b=Z.formBarsOf(e);
      assert(!!Z.SEC_TYPES[Z.formType(e)],'a form asks for a section type that does not exist'+t);
      assert(b>0&&b%4===0&&b<=16,'a form asks for a section of '+b+' bars'+t);
    });
    const types=form.map(Z.formType);
    assert(types[0]==='Intro','a form does not open with an intro'+t);
    assert(types[types.length-1]==='Outro','a form does not end with an outro'+t);
    assert(types.indexOf('Verse')>0,'a form has no verse'+t);
    assert(types.indexOf('Chorus')>0,'a form has no chorus'+t);
    assert(bars>prevBars,'rung '+n+' of the ladder is not longer than the one under it'+t);
    prevBars=bars;
    // and the sections it builds are sections the arranger itself could hold
    const secs=Z.formSections(form);
    assert(secs.length===form.length,'a form built '+secs.length+' sections out of '+form.length+t);
    assert(secs.reduce((a,s)=>a+s.bars,0)===bars,'the sections of a form do not come to its length'+t);
    secs.forEach((s,i)=>{
      const base=Z.sectionOf(Z.formType(form[i]));
      assert(s.type===base.type&&s.part===base.part&&s.energy===base.energy&&s.sweep===base.sweep&&s.fade===base.fade,
        'a built section is not its type'+t);
      assert(s.bars===Z.formBarsOf(form[i]),'a built section is '+s.bars+' bars, not '+Z.formBarsOf(form[i])+t);
      assert(JSON.stringify(Z.normSection(s))===JSON.stringify(s),'a built section is not one the arranger holds'+t);
    });
  }
  // at every tempo the slider can reach: the closest rung there is, near the length it promises, and always
  // longer than the preset below it
  for(let bpm=60;bpm<=180;bpm++){
    let last=0;
    for(const p of Z.SONG_LENGTHS){
      const form=Z.songForm(p.seconds,bpm),bars=Z.formBars(form),secs=Z.formSeconds(form,bpm),
        mine=JSON.stringify(form),t=' ['+p.id+' at '+bpm+' bpm]';
      let best=Infinity,found=-1;
      for(let n=0;n<=Z.FORM_MAX_STAGE;n++){
        const f=Z.formAt(n),e=Math.abs(Z.formSeconds(f,bpm)-p.seconds);
        if(JSON.stringify(f)===mine)found=e;
        if(e<best)best=e;
      }
      assert(found>=0,'a song length built a form that is not on the ladder'+t);
      assert(found<=best+1e-9,'a song length picked a form '+Math.round(found)+' s out when one '+Math.round(best)+' s out was there'+t);
      assert(Math.abs(secs-p.seconds)<=p.seconds*0.15,'a '+p.label+' song came to '+Z.clockOf(secs)+t);
      assert(bars>last,'a '+p.label+' song is no longer than the preset below it'+t);
      last=bars;
      assert(Z.clockOf(secs).indexOf(':6')<0,'a song length reads '+Z.clockOf(secs)+t);
    }
  }
  // the scale lock holds over a rebuilt arrangement: a preset only swaps the sections, so every section of
  // every form still generates chords, a melody, an arp and a bass line that are all in the key
  for(const scale of ['dorian','major','hungMinor'])for(const root of [0,7]){
    const sc=Z.SCALES[scale],cs=Z.chordScaleOf(scale);
    const melPcs=pcsOf(root,sc.steps),chPcs=pcsOf(root,cs.steps);
    for(const p of Z.SONG_LENGTHS){
      const S={chords:'FORM01',lead:'FORM01l',arp:'FORM01a',bass:'FORM01b',drums:'FORM01d'};
      Z.formSections(Z.songForm(p.seconds,124)).forEach((s,i)=>{
        const t=' ['+Z.NOTE_NAMES[root]+' '+sc.name+' · '+p.id+' · '+s.type+' '+i+']';
        const cfg={root,scale,energy:55+s.energy,leadEnergy:55,evolve:true,sevenths:true,gate:0.7,hook:!!s.hook};
        const tr=Z.generateTrack(cfg,S,s.part,0);tracks++;
        assert(tr.chords.reduce((a,c)=>a+c.bars,0)===Z.BARS,'a section of a rebuilt song does not fill its chord loop'+t);
        tr.chords.forEach(c=>c.notes.forEach(m=>assert(chPcs.has(((m%12)+12)%12),'chord tone outside scale'+t)));
        tr.lead.forEach(e=>assert(melPcs.has(((e.midi%12)+12)%12),'lead note outside scale'+t));
        tr.arp.forEach(e=>assert(Z.chordAt(tr.chords,e.step).pcs.has(((e.midi%12)+12)%12),'arp note not in the chord under it'+t));
        tr.bass.forEach(e=>{assert(chPcs.has(((e.midi%12)+12)%12),'bass note outside scale'+t);
          assert(e.midi>=Z.BASS_LO&&e.midi<=Z.BASS_HI,'bass note '+e.midi+' out of register'+t)});
      });
    }
  }
  // and a rebuilt arrangement travels with the song like everything else
  {
    const p=Z.demoProject('chill'),form=Z.songForm(300,128);
    p.state.bpm=128;p.state.sections=Z.formSections(form);p.state.sel=0;
    const want=p.state.sections.map(s=>s.type+':'+s.bars).join(','),hash=Z.linkHash(p),back=Z.linkDecode(hash);
    assert(!!back,'a rebuilt arrangement did not survive a link'+tag);
    assert(back&&back.state.sections.map(s=>s.type+':'+s.bars).join(',')===want,'a rebuilt arrangement came back changed'+tag);
    assert(hash.length<Z.LINK.maxBytes,'a five-minute song made a link of '+hash.length+' characters'+tag);
  }
}
