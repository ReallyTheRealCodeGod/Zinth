...
foreach ($f in 'theory','engine','ui-core','ui-panels') { $parts += '<script>'; $parts += (Get-Content "src/$f.js" -Raw); $parts += '</script>' }
[IO.File]::WriteAllText("$PSScriptRoot/index.html", ($parts -join "`n"), $utf8)

New-Item -ItemType Directory -Force tests | Out-Null
$check = @('<title>Zinth theory check</title>', $fonts, '<script>', (Get-Content src/theory.js -Raw), '</script>', '<script>', (Get-Content tests/check.js -Raw), '</script>')
