...
Zinth opens as an instrument in the spirit of the OP-1: one screen at a time, four knobs whose meaning changes with the screen, and the keys underneath. Drag a knob up or down, spin the wheel over it, or focus it and use the arrow keys. Three modifiers give the same four knobs a wider reach without putting anything else on the screen: <kbd>Shift</kbd> turns a quarter as fast, so a knob places as finely as a slider; <kbd>Alt</kbd> makes the turn momentary, springing the knob back to where it started the instant you let go, which turns a knob into a performance control — every knob but Mood, which rerolls the track and so has nothing to come back to, and says so rather than pretending; and a double-click puts a knob back to its default — the value the current mood asks for, wherever the mood has an opinion, and the app's own default otherwise. Mood, Key and Sound have no honest default and say so rather than guessing.

| Screen | What you see | The four knobs |
| --- | --- | --- |
...
| **Mix** | Five faders with mute, lock and roll | Volume · Warmth · Humanize · Tone |

Nothing in the interface carries a hue. The surfaces are white through grey to near-black, hairline rules divide them, and what would have been colour is tone instead: the five layers are five steps of one grey ramp, in the order lead, arp, chords, bass, drums, so a mixer or a piano roll reads as a tonal scale. Anything selected is simply ink — filled, outlined or inverted. Both themes are built from the same tokens at the top of `src/styles.css`, and the canvases read those tokens too, so a change there moves the whole app.
...
Under the knobs, on every screen, sits the **punch-in** row: seven momentary effects — low sweep, high sweep, gate at an eighth, gate at a sixteenth, crush, delay throw and wash. Hold a pad or its key, <kbd>Z</kbd> to <kbd>M</kbd>, and the mix does it for as long as you hold; let go and the song is exactly where it should be. The same strip is in Studio, between the mixer and the roll — it is one set of pads that the device borrows for as long as it is on screen, so there is nothing to keep in step.

Chord pads and keys sit under every screen, and so do the chord box's own controls: **7ths**, **Hold** and **Bass**, and the tray that collects what you play with **→ A verse**, **→ B chorus** and **Clear** beside it. Improvise a progression on the pads and one press makes it the song's. **Studio**, in the top bar, opens everything else: the chord cards and editor, the arrangement inspector and length presets, swing, humanize, transitions and warmth, saved songs, and the full Sound panel with every parameter of every voice, the drum grid and the master EQ. The **Song ▾** menu holds keep, copy link, save and open, and the WAV, MIDI and stems exports.
...
- **Lock and reroll.** New track rerolls only unlocked layers. Each layer has its own roll.
...
- **Record your melody, your arp or your bass line.** Press Rec, choose the layer to record into, and play the letter keys over the looping section. Your notes replace that generated layer, snapped to the grid. The keys sound the layer you are playing into and land where it sings: the arp an octave up, the bass in its own register, always in key. A metronome click is one button away. Arming Rec always leaves you somewhere a take can be heard: it takes the section you are actually playing over rather than whichever one is selected, loops it so the take comes straight back round, and switches the layer on for that section if it happened to be off — an Intro and an Outro do not play the lead — saying so in the status line, one <kbd>Ctrl</kbd>+<kbd>Z</kbd> away. Every note you record names itself and counts the take, the Song screen shows how many notes are yours with a **Clear take** beside it, and ↺ beside the lane name brings the generated notes back.
...
| Shift + knob drag | Fine turn, a quarter as fast |
| Alt + knob drag | Momentary: the knob springs back on release |
